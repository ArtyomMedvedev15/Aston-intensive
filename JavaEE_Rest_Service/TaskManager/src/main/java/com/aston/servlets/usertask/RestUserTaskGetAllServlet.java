...
@Slf4j
@WebServlet(urlPatterns = "/api/v1/user/task/all")
public class RestUserTaskGetAllServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
         try {
            List<UserTaskFullDto> userTaskFullDtosList =
                    userTaskServiceApi.getAllUsersTask();
            ...
            log.info("Get user task with id");
            resp.getWriter().write(userTaskListJson);

        } catch (SQLException e) {
            resp.setStatus(HttpServletResponse.SC_BAD_REQUEST);
            PrintWriter out = resp.getWriter();
            out.println(String.format("Cannot get user task get error %s in %s", e.getMessage(), new Date()));
        }
    }
}
